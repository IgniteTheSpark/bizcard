...
description: >
  ...
  (source_text + user_text + session_id + input_id) and handles all todo
...
2. Call `tool_query_asset` with `asset_type="todo"` and `contains=<keyword>` to find candidates.
...
2. Call `tool_query_asset` with `asset_type="todo"` and `contains=<keyword>`.
...
```
→ query_asset(asset_type="todo", contains="饭局")
→ find "有一个吃饭的饭局" todo
...
```
→ query_asset(asset_type="todo", contains="刘洋")
...
```
→ query_asset(asset_type="todo", contains="开会")
