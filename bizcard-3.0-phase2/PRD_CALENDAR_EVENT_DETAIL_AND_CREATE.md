...
- **已完成代办**：在 **单日**、**日程** 中始终与未完成同列展示；完成态以 **删除线**（及置灰等）区分，不提供单独显隐开关。
...
- **已完成代办**：在单日/日程长期展示，与未完成同列；通过删除线与置灰区分状态，不单独配置显隐。
...
3. 侧边栏支持：单日/日程切换；「我的」内支持 **日程 / 代办** 两类显隐控制（已完成代办随代办一起展示）。
...
| v1.5 | 2026-04-08 | 口径历史版本：曾定义“已完成代办始终展示” |
| v1.6 | 2026-04-08 | §4 独立文档引用：第三方接入 **不含 Onboarding**，与 `PRD_CALENDAR_INTEGRATION_ENTRY_AND_MANAGEMENT.md` v1.1 对齐 |
| v1.7 | 2026-04-09 | 与最终 Demo 对齐：侧边栏「我的」新增 **显示已完成代办** 开关；单日/日程同步生效 |
| v1.8 | 2026-04-09 | 口径回收：已完成代办在单日/日程 **长期展示**（删除线区分），移除侧边栏「显示已完成代办」开关 |
