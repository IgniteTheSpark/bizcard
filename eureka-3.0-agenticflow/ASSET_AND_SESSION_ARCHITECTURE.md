...
**v0.9 相对 v0.8 的核心变更**：
1. 新增内置 `asset_type`：**`misc`**（杂项 / 未归类捕获），用于承接 Skill 无法归入既有类型的硬件闪念产出；用户可删除任意 Asset（含 misc）。
2. **硬件闪念**：每一轮**语音**输入经 Skill 判定产出类型（`todo` / `idea` / `note` / `misc` / …）；**不默认合并**多轮输入为单条 Asset（一轮输入对应零条或一条结构化产出，按 Skill 定义）。
3. **Workspace**：增加 **Misc** 分类入口，承载全部 `misc` 类型 Asset 的二级列表。
4. **普通 Ask Agent（general / anchored）**：Agent 回复支持用户主动沉淀——**设为 Note / 设为 Idea / 设为 Misc**，或统一 **「设为 Asset」** 后再选择类型（产品二选一即可）；与「仅保存为 Note」的单按钮相比，类型由用户选定。
...
7. Agent 回复末尾新增"保存为 Note"功能（v0.9 扩展为多种沉淀类型，见 §9.4）。
...
| `misc` | 杂项捕获：闲聊、无法归类片段、Skill 兜底产出；可删改 | 硬件闪念（主）/ 对话沉淀（用户选择）/ 手动 |

`note_type` 子类型示例：`meeting_summary`（音频解析）/ `pdf_summary`（PDF 解析）/ `manual`（手动）/ `conversation_note`（对话生成）。

**`misc` 与 Skill**：对话管线中由 dispatcher / 子 Skill 判定产出类型；当内容**不宜**写入 `todo` / `idea` / `note` / `contact` 等既有类型时，**统一降级为 `misc`**，保证语义单一。**不**对多轮语音做默认合并：每一轮输入独立走 Skill，**至多产生一条**结构化 Asset（类型由 Skill 输出，可为 `misc` 或其他）。若产品约定「每轮硬件语音必落一条」，则无法归类时一律写入 `misc`（载转写或摘要正文）。

**扩展 asset_type 示例**：`expense`（记账，含金额/地点）、`vocabulary`（单词）、`health_log`（健康记录）。新增时后端不变，只需定义 Skill + 前端模板。

**删除**：所有 `asset_type`（含 `misc`）均支持用户删除；删除后 Workspace 列表与来源跳转按常规失效处理。
...
**硬件语音闪念（单轮规则）**：
- 每轮 **turn_input**（一次拾音 / 一次 ASR 结果）进入 **conversation-skill** → 匹配子 Skill，决定产出 `asset_type`（含 **`misc` 兜底**）。
- **不做跨轮默认合并**：上一轮与下一轮的产出为彼此独立的 Asset（除非未来单独定义「用户显式合并」功能，不属于默认行为）。
...
| `misc-sub-skill` | 兜底：无意义对话、无法归类、或产品定义为杂项的文本 → Misc Asset |
...
│
└── Workspace 区
      按 asset_type 分类入口（Notes / Ideas / Todos / **Misc** / Expenses…）
      点击分类进入二级列表，支持增改删（含删除 misc）
      手动创建入口在二级列表页
...
#### Agent 回复气泡（左侧）
- 文字回复 + 内嵌结果卡片（Todo / Note / Idea / Contact / Misc）
- 结果卡片直接跟在文字下方，不在底部积累
- **用户主动沉淀（general / anchored）**：每条（或选定段）Agent 文字回复末尾提供沉淀入口，**不得**在未经用户选择类型的情况下写入 Workspace（与硬件闪念「Skill 自动产出」区分）。实现二选一即可：
  - **多按钮**：「设为 Note」「设为 Idea」「设为 Misc」；或
  - **单入口**：「设为 Asset」→ Sheet 内选择 `note` / `idea` / `misc`（及后续扩展类型）。
- 点击后在 `assets` 表创建对应 `asset_type` 的记录，`source_type = "session"`，并挂到 §9.4 表格规定的 Workspace / File / Contact 落点。
...
- **硬件语音**：每轮由 Skill 判定类型并创建 Asset（可含 **`misc`**）；**不默认合并**多轮（见 §8.2）。
- 产出 Asset 只进 Workspace（`context_tag: "flash"`，无 context）；**Misc** 与其他类型一致，经 Workspace 的 Misc 入口进入二级列表
...
13. **`misc` 与 Skill 兜底**：无法归入既有类型的对话产出写入 `misc`；Workspace 提供 Misc 容器；所有 Asset（含 misc）可删除。
14. **Ask 沉淀由用户选类型**：App 内 Ask 的回复不落 Workspace，直至用户执行「设为 Note / Idea / Misc」或「设为 Asset」后选定类型。
